...
public class UserRepositoryJdbcTemplateImpl extends JdbcTemplate implements UsersRepository {
    ...
    public void save(User entity) throws SQLException {
        ...
        savePassword(entity.getEmail(), new UsersServiceImpl().generatePassword());
    ...
    public Optional<User> findByEmail(String email) {
        try {
            User user = super.queryForObject(String.format("SELECT * FROM %s WHERE email = '%s'", tablename, email),
    ...
    public void savePassword(String email, String password){
        String command = String.format("INSERT INTO passwordsByEmail (email, password) VALUES ('%s', '%s');", email, password);
        super.update(command);
    }

    public String findPasswordByEmail(String email) {
        String password;
        password = super.queryForObject(String.format("SELECT * FROM passwordsByEmail WHERE email = '%s';", email), (rs, rowNum) -> new String(rs.getString("password")));
        return password;
    }
}
